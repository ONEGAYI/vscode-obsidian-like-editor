...
- **Quick actions**: the pen button in the editor toolbar expands an in-flow formatting bar. Common formats, a heading-level menu, and table creation act on the original selection; the expanded state is remembered. Formatting writes are unavailable in reading mode.
...
| Add table rows & columns | Hover near the bottom/right edge to reveal an add strip, or use the "表格：…" commands |
