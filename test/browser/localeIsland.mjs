// 浏览器回归共用的语言数据岛构建（#94）：编辑器/设置页文案经 t() 取词后，
// 页面无岛时取词回退键名、中文断言全挂。此模块经 esbuild 打包生产侧
// buildLocaleIslandHtml + zhCn 生成与宿主 HTML 生成点同源的岛（转义规则
// 一致），harness 注入 setContent 页面，fixture 入口 bootLocaleFromDocument
// 装配——与 webview 首帧同路径，不旁路生产机制。zhCnMessages 一并导出，
// 供断言与字典同源取值（不再复制字面量）。
