...
状态：工单 #6 交付物，2026-09-23；#8 补 span 级映射与阅读语义标签结构（2026-09-24）；#9 补任务勾选交互类（2026-09-24）；#10 补链接/图片映射（2026-09-24）；#12 补表格映射（2026-09-24）；#11 补双链映射（2026-09-24）；#42 补实时预览表格网格入口（2026-09-24）；#55 移除 live 标题行左缘竖线及其强调色变量（2026-09-25）；#65 补大纲面板条目与行内透传入口、标题层级色变量族（2026-09-25）。依据 [ADR-0004](../adr/0004-stable-styling-contract.md)。
...
右侧栏大纲面板为本项目自有 UI（Obsidian 的大纲属应用级 DOM，不参与兼容承诺）。#65 起条目按白名单行内标记结构渲染（语义元素 + 稳定类名双入口），层级颜色与正文标题引用同一变量族（见「公开 CSS 变量」的 `--vsidian-heading-color-*` 族）；设计哲学（结构装饰与正文主题同源、强调语义只认显式标记、透传集合 = 正文已支持的行内标记子集）已落档仓库 `AGENTS.md`「约定」。
