...
- **右侧栏与大纲**：编辑器顶栏齿轮直达设置；右侧栏按钮一键展开收起（细线／粗线两态图标，状态跨面板记忆），左缘可拖拽调整宽度（双击恢复默认，方向键微调，宽度跨面板记忆）。「大纲」面板：标题层级实时同步（ATX/Setext 语义、代码围栏与 frontmatter 伪标题不计入）；点击条目跳转正文对应标题（双模式），常驻高亮指示当前所在章节；六档「结绳」滑块统一展开深度，条目箭头手动折叠，滚动自动展开当前路径；工具条提供跳转末尾、重置与标题搜索（片段级高亮）；标题行内样式（粗体、斜体、高亮、行内代码、删除线）与主题层级配色同源透传；右键菜单提供结构命令、五项复制、调整层级、重命名（保留行内标记）与整段删除；拖拽标题按「控制域」整段重排，控制域外内容零误伤，一次撤销完整回滚。
...
- **格式命令**：实时预览中可从命令面板切换粗体、斜体、高亮、删除线、行内代码、标题、列表、引用、代码块与链接，或清除行内格式；可插入公式、双链与分割线。显式选区优先，无选区的行内格式作用于当前词；一次操作可一次撤销。
- **快速操作条**：编辑器顶栏的笔形按钮可展开流内格式工具条；常用格式、标题层级弹出菜单、插入公式、插入分割线与建表入口直接作用于原选区，展开状态会记住。阅读模式下格式写操作不可用。
- **快捷键管理**：设置页「快捷键」分页集中管理全部可绑定操作（格式、表格、查找、视图切换、大纲等）；按键录制添加绑定（支持两段键），逐条删除、一键清空、单项或全部恢复默认，清空选择持久保留。插件内部冲突按「键位相同且生效范围重叠」判定；写操作快捷键仅在 Live 编辑正文时拦截，源码模式与设置页输入不受影响。
- **中英双语界面**：编辑器、设置页、通知与命令标题全部经语言包取词；设置页「常规 → 界面语言」提供自动／简体中文／English 三档，默认跟随 VSCode 显示语言，切换即时生效。
...
- **图形化代码块交互**：渲染图形右上角悬停显示按钮组——「编辑源码」（仅实时预览，光标落入围栏显形源码）与「弹窗预览」（双视图，全屏浮层放大浏览：滚轮/键盘缩放、拖拽平移、一键回到整图、按当前源码刷新，缩放按矢量重排文字不糊）；点击图形本体不进入编辑，编辑入口收敛到按钮。图表可导出为 SVG 与 PNG 文件（宿主另存为对话框落盘；环境不支持 PNG 光栅化时自动降级为仅 SVG）。按钮组、弹窗与导出由渲染器注册表统一驱动，后续新增同类图表语言登记即继承全部交互。
...
| 快捷键管理 | 设置页「快捷键」分页：搜索定位操作，录制新键位，支持清空与恢复默认 |
...
| 查找 | Ctrl+F / Cmd+F |
| 右侧栏 / 大纲 | 顶栏右侧切换按钮展开侧栏，展开后点「大纲」；拖动侧栏左缘调整宽度（双击恢复默认） |
...
当前设置项：**界面语言**（默认自动）——跟随 VSCode 显示语言，可固定简体中文或 English，切换即时生效。**显示行号**（默认开启）——实时预览左侧行号栏显示源文件行号，表格段显示段首行号；阅读模式始终无行号。**代码块卡片 / 卡内行号 / 复制按钮 / 语法高亮**（默认开启）——卡片整体外观及三项子开关，语法高亮独立于卡片（卡片关闭时朴素围栏同样着色）。
