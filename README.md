...
- **快速操作条**：编辑器顶栏的笔形按钮可展开流内格式工具条；常用格式、标题层级弹出菜单、插入公式与建表入口直接作用于原选区，展开状态会记住。阅读模式下格式写操作不可用。
