...
- 本联合分支自动化回归：857 项 Vitest 单测、11 项启动器契约测试，以及开发态与 VSIX 安装态各 81 项真实 VSCode 1.86.2 宿主集成用例通过；空白格组合提交后立即关闭的宿主队列竞态及已确认编辑重传误报均有单测回归保护，人工验收另见[验证清单](docs/specs/manual-verification.md)。
