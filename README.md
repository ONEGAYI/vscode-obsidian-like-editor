...
- **代码块卡片**：围栏代码块在光标离开时收起为卡片——语言头部横带（彩色徽标 + 显示名）、卡内行号、悬停复制按钮（按文档换行风格写入，CRLF 文档得 CRLF）与折叠 chevron；内置 17 语言语法高亮引擎（另含纯文本，支持语言别名，明暗两套 VSCode 风格色板，未识别语言回退纯文本），实时预览与阅读观感一致；mermaid 等渲染型围栏编辑态同样显示卡片外壳与行号。
...
| 代码块卡片 | 光标离开围栏自动收起为卡片；悬停头部复制整块；点 chevron 折叠，光标进入展开 |
| 设置 | 「Vsidian: 打开设置」，或编辑器顶栏齿轮按钮 |

当前设置项：**显示行号**（默认开启）——实时预览左侧行号栏显示源文件行号，表格段显示段首行号；阅读模式始终无行号。**代码块卡片 / 卡内行号 / 复制按钮 / 语法高亮**（默认开启）——卡片整体外观及三项子开关，语法高亮独立于卡片（卡片关闭时朴素围栏同样着色）。
...
- 表格列数不一致时显示可编辑源码而非网格。
- 代码块卡片：超过 4096 行的围栏跳过语法着色（回退纯文本卡片）；语言徽标为字形徽标（非矢量 logo）。
